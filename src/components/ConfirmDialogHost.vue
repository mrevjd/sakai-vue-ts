<script setup lang="ts">
    ...
    // Both shapes stay mounted and each owns its `open`: swapping them with v-if would unmount the closing one before it animates out.
    const dialogOpen = computed(() => confirmState.visible && !confirmState.options?.target);
</script>

<template>
    <AlertDialog :open="dialogOpen" @update:open="onOpenChange">
        <AlertDialogContent>
            <AlertDialogHeader>
            ...
            <AlertDialogFooter>
                <Button :variant="confirmState.options?.rejectVariant ?? 'outline'" data-testid="confirm-reject" @click="rejectConfirm">{{ confirmState.options?.rejectLabel ?? 'No' }}</Button>
