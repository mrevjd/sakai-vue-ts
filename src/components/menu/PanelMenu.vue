    ...
    <div :class="cn('flex flex-col', props.nested ? 'gap-0.5 pl-4' : 'gap-2', props.class)" data-slot="panel-menu">
        <template v-for="(item, index) in props.model.filter(isVisible)" :key="index">
            ...
            <button
                ...
                :class="cn('flex w-full items-center rounded-md px-3 py-2 text-left text-sm hover:bg-muted focus-visible:outline-none disabled:opacity-50', !props.nested && 'rounded-lg border border-border bg-card px-4 py-3 font-semibold')"
                :data-slot="props.nested ? 'panel-menu-item' : 'panel-menu-header'"
